...
func buildTable(containers []docker.Container, width int) table.Model {
	const (
		...
		stateW      = 9
		runningForW = 13
		overhead    = 16 // 7 cols × 2 (cell padding) + 2 (outer rounded border)
	)

	// Seed with header title lengths as minimums.
	...
	// Cap columns that can get arbitrarily long.
	nameW = min(nameW, 30)
	...
	// Scale all four columns proportionally to fill the available width exactly.
